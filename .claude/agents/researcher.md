---
name: researcher
description: Use for long-context analysis, multi-document synthesis, RAG-style extraction from PDFs / HWP / large dumps. Backed by Gemini (large context window).
...
## Invocation
You are typically driven by `gemini` via `scripts/invoke-gemini.mjs`. Long inputs go on stdin or as `--input <file>`.
